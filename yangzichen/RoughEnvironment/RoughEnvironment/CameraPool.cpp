...
namespace pool
{
	CCameraPool::CCameraPool() : mPos(0.0f, 15.0f, 0.0f), mLook(0.0f, 0.0f, 1.0f), mUp(0.0f, 0.1f, 0.0f), mRight(1.0f, 0.0f, 0.0f)
	{
	}


	CCameraPool::~CCameraPool()
	{
	}

	void CCameraPool::LRMove(float n)
	{
		this->mPos += this->mRight * n;
	}

	void CCameraPool::UDMove(float n)
	{
		this->mPos += this->mUp * n;
	}

	void CCameraPool::FBMove(float n)
	{
		this->mPos += this->mLook * n;
	}

	void CCameraPool::Initialize(float posX, float posY, float posZ,
	...
	void CCameraPool::Initialize(D3DXVECTOR3 pos, D3DXVECTOR3 look, D3DXVECTOR3 up)
	{
		this->mPos = pos;
	...
	void CCameraPool::XRotation(float angle)
	{
		D3DXMATRIX T;
	...
	void CCameraPool::YRotation(float angle)
	{
		D3DXMATRIX T;
	...
	void CCameraPool::ZRotation(float angle)
	{
		D3DXMATRIX T;
	...
	void CCameraPool::SetPosition(D3DXVECTOR3 pos)
	{
		this->mPos = pos;
	}

	void CCameraPool::SetPosition(float x, float y, float z)
	{
		D3DXVECTOR3 pos(x, y, z);
		this ->mPos = pos;
	}

	D3DXVECTOR3 CCameraPool::GetPosition()
	{
		return this->mPos;
	}

	D3DXVECTOR3 CCameraPool::GetLook()
	{
		return this->mLook;
	}

	D3DXVECTOR3 CCameraPool::GetRight()
	{
		return this->mRight;
	}

	D3DXVECTOR3 CCameraPool::GetUp()
	{
		return this->mUp;
	}

	D3DXMATRIX CCameraPool::GetViewMatrix()
	{
		D3DXMATRIX v;
	...
	void CCameraPool::Response(DWORD time, PINPUT input)
	{	
		...
		if (input->MouseButtonDown(Mouse::RIGHT_BUTTON))
		{
			this->UDMove(input->GetMousePosDelta().y * 0.01);
		}
